...
export default async function ProjectDetailPage({ params }: { params: { slug: string } }) {
  ...
  try {
    const res = await fetch(`https://backend-shohanur-1.onrender.com/gallery/?slug=${slug}`, {
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        ...
        {project.details && (
          <article className="prose prose-lg sm:prose-xl max-w-none prose-headings:text-gray-900 prose-strong:text-purple-700 prose-a:text-purple-600 hover:prose-a:underline">
            <h1 className="text-lg sm:text-xl text-gray-600 font-bold max-w-4xl leading-relaxed mb-5">
              {project.description}
            </h1>
